...
if ( empty( $argv[1] ) ) {
	echo "Usage: php svn-publisher.php \$username\n";
	die;
}
$username = $argv[1];

if ( strpos( $cwd, $dir ) ) {
	echo "Don't run this command inside the dropp repo please.\n";
	die;
}
...
`rm -rf update-pdf-packages.php .idea .git .gitignore composer.json svn-publisher.php README.md CONTRIBUTING.md`;
`rm -rf tags node_modules package.json package.lock webpack.mix.js`;
...
exec( 'svn commit --username ' . $username . ' -m "Synchronized trunk with master branch from Github"', $output, $result );
if ( $result ) {
